"""
xlsx_lock.py — Кооперативный file-lock для xlsx-реестров.
...
def is_xlsx_busy(xlsx_path):
    ...
    resolutions-daemon видит что регистратор пишет в реестр, пропускает
    его на этом тике и берётся за другие файлы.

    NB: это просто подсказка. Между проверкой и реальной попыткой взять
    ...
    try:
        ...
        return False  # .lock не существует — файл свободен
